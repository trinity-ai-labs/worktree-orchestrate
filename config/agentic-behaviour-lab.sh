...
# Markdown-only work (docs/, experiments/ findings) skips env + install.
DOCS_BRANCH_PREFIX="docs/"
DOCS_NOTE="experiments/ and docs/ markdown needs no install; run 'pnpm install && pnpm build' by hand if the change touches TypeScript (check needs the workspace built)."
