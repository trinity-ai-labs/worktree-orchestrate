...
DOCS_NOTE="format changed markdown with 'git diff --name-only | xargs node_modules/.bin/prettier --write' and check drift with 'pnpm docs:check' — both run in-worktree (node_modules is symlinked). Run a full 'pnpm install' by hand only if the change touches TypeScript (e.g. registering a chapter in trinityailabs.com/lib/content/docs-structure.ts) or needs turbo's 'pnpm format'/'pnpm gate'."
