...
- **Check substance, not just the file count.** Staying in scope ≠ doing it right — a diffstat tells you *where* the agent is editing, not *whether the approach is correct*. On any slice with real design subtlety, periodically **read the actual diff of the 2–3 highest-risk files** (`git -C <wt> --no-pager diff HEAD -- <file>`), not just `--stat`, and confirm the agent is following the intended approach (or genuinely improving on it). **Grep the diff for the specific anti-patterns the brief banned** (e.g. a forbidden call leaking into the wrong layer: `git -C <wt> diff HEAD -- <commit-time files> | grep -nE '<bannedSymbols>'`). A wrong-but-in-scope approach is exactly what file-counting misses, and catching it mid-flight saves a whole wasted run. Keep it light — the risk files, not the entire diff.
