...
- **The project's `BRIEF_CONVENTIONS`** from its config (`~/.worktrees/config/<project>.sh`) plus anything in the repo's `AGENTS.md` — framework skills, compat policy, comment style, etc. (For Trinity: pre-launch forward-only with no backwards-compat shims; comments explain the mechanism.)
- **When the task touches a framework with a reference skill, instruct the agent to INVOKE that skill (via the Skill tool) as its first step — not merely "use it."** For Trinity, any slice with Effect-TS code (services, layers, HTTP routes, typed errors) must open with "Step 0: invoke the `effect` skill." This matters most for Sonnet agents, which won't reach for the skill on their own — spell it out as an explicit first step in the brief.
...
- **Run the `/simplify` skill over the changes while they are still UNCOMMITTED**, fold the fixes in, then **run the project's `GATE_CMD`** to green. `/simplify` reads the *uncommitted working-tree diff* — against a clean tree (everything already committed) it has nothing to act on and silently no-ops, reporting "nothing actionable" when in truth nothing was ever reviewed. So the simplify pass must precede the commit of the code it covers; never commit-everything-then-simplify.
- **Don't be lazy — fix every issue your change surfaces, in scope or not, and commit it SEPARATELY.** A gate that *exits* green while still printing warnings is NOT done: the bar is zero lint/type warnings AND errors on every file you touch, including the ones your own change surfaces — residual unused imports left behind after you move code to a sibling module, a path-keyed lint allowlist (sanctioned-git / ambient-identity rules) that must follow the relocated code to its NEW file path, a stray type/lint nit in a file you had to open. Leave the tree cleaner than you found it. Land each such opportunistic / out-of-scope fix in its OWN commit with a mechanism message (e.g. `chore(lint): prune imports the split left unused`), kept apart from the task's commits so the orchestrator can keep or drop it independently — never fold an unrelated fix silently into a task commit, and never hand back a "passing" PR that still carries warnings for the orchestrator to mop up.
...
**Commit as you go, in logical blocks.** Don't accumulate the whole task into one end-of-run commit — commit each self-contained step (a type + its plumbing, a behavior + its tests, one cohesive refactor) as you finish it, with a message that explains the mechanism. We never squash, so these commits survive into history and let the orchestrator review the PR block-by-block. The gate needn't be green at every intermediate commit, but it MUST be green at the final one. (Because `/simplify` only sees *uncommitted* changes — see the next paragraph — run your simplify pass over the working tree *before* committing the code it covers; don't commit everything and then simplify a clean tree.)

**Run the `/simplify` skill while your changes are still UNCOMMITTED** (reuse / simplification / efficiency / altitude cleanups — quality only, not bug-hunting), fold the fixes in, then **run the project's `GATE_CMD`** and make it pass — so the PR is already simplified and green when the orchestrator reviews it. ⚠️ `/simplify` acts on the *uncommitted working-tree diff*: commit everything first and then run it and it sees an empty diff, finds nothing, and silently does nothing — the review never happened. So simplify the working tree *before* you commit the code it covers (run it over the whole change set before your first commit, or per-block before each block's commit); never commit-all-then-simplify. Then push all your commits, open a PR targeting the integration branch, and report back: PR URL, files changed, gate result, tests touched, and anything ambiguous. **Never rebase. Never merge your own PR.**
...
- **⛔ `/simplify` only acts on UNCOMMITTED changes.** It reads the working-tree diff, so against a clean tree it silently no-ops — finding "nothing actionable" because there is nothing to see, not because the code is clean. Always run it *before* committing the code it reviews; never commit-everything-then-simplify (that "reviews" an empty diff and the simplification never happens). The order is load-bearing, not cosmetic.
